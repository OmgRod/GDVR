...
bool OpenXRManager::initialise() {
    ...
    // ---------------------------------------------------------------------------
    // Obtain the Android Activity context
    // ---------------------------------------------------------------------------
    //
    // GeometryDashVRBridge.cpp (in liblauncherfix.so) exports a plain-C symbol
    // geode_vr_getActivityContext() that returns the GeometryDashVRActivity
    // instance stored during nativeOnCreate(). This is the correct Activity for
    // OpenXR: the runtime uses it to attach to the Android lifecycle and to
    // validate the application window.
    //
    // We resolve the symbol at runtime so the mod does not need to link against
    // the launcher. RTLD_DEFAULT searches all already-loaded shared libraries,
    // which includes liblauncherfix.so since the launcher loaded it before Geode.
    ...
    // Fallback: if the launcher bridge symbol is missing or returned null,
    // try Cocos2dxActivity.getContext() as a last resort. This was the old
    // approach and may return the wrong Activity class on some GD versions.
    if (!activity) {
